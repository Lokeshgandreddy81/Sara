            ...
            <div className="team-portfolio bg-[#fafafa] dark:bg-[#1f1f1f]">
                ...
                <div className="members-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 p-4">
                    {membersData.map((member) => (
                        ...
                        <div key={member.id} className="member-card relative bg-[#f0f0f0] dark:bg-[#262626] rounded-xl shadow-lg p-4 md:p-6 hover:shadow-2xl transition-shadow duration-300 ease-in-out">

                            {/* Top-right LinkedIn icon */}
                            <a href={member.Linkeddin} target="_blank" rel="noopener noreferrer" className="absolute top-4 right-4 text-blue-600 hover:text-blue-800">
                                <Linkedin  className='dark:text-gray-300 h-5 w-5 '
                                />
                            </a>
